...
class TransformerMEncoder(nn.Module):
    ...
    def forward(
        ...
        # print("size of x after atom_feature: " + str(x.size()))

        if perturb is not None:
            x[:, 1:, :] += perturb

        # print("1: size of x" + str(x.size()))
        ...
        if self.molecule_3d_bias is not None and not (batched_data["pos"] == 0).all():
            ...
            # print("2: size of x" + str(x.size()))

        if self.embed_scale is not None:
            x = x * self.embed_scale

        # print("3: size of x" + str(x.size()))

        if self.quant_noise is not None:
            x = self.quant_noise(x)

        # print("4: size of x" + str(x.size()))

        if self.emb_layer_norm is not None:
            x = self.emb_layer_norm(x)

        # print("5: size of x" + str(x.size()))

        x = self.dropout_module(x)

        # print("6: size of x" + str(x.size()))
        # account for padding while computing the representation

        # B x T x C -> T x B x C
        x = x.transpose(0, 1)

        # print("7: size of x" + str(x.size()))
        ...
        # print("8: size of x" + str(x.size()))

        atom_output = None
        if delta_pos is not None:
            atom_output = self.atom_proc(x[1:, :, :], attn_bias[:, :, 1:, 1:], delta_pos)
            # print("Shape of atom output before mask 3d: " + str(atom_output.shape))
            if mask_3d is not None:
