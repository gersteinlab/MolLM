...
lr = 0.1
lr_scheduler_steps = 400
lr_scheduler_gamma = 0.1


def run_z_optimize(model, atomic_num_list, input_text, z, num_steps, original_mol_rep, alpha=0.5):
    ...
    while run[0] <= num_steps and skipped_in_a_row < 10:
        ...
        try:
            ...
            if torch.isnan(prompt_loss) and torch.isnan(original_mol_loss):
                print(loss)
                break
        ...
        run[0] += 1
        if run[0] % 100 == 0:
            print(f'Optimization Step {run} Loss ({loss:.4f}) = prompt ({prompt_loss:.4f}) + {alpha} * original_mol ({original_mol_loss:.4f})')
...
# Generation
def generate(input_smiles, input_text, attempts=10) -> List[str]:
    global mu, temperature, alpha, total_steps

    smiles_to_return = []
    try:
        z = smiles_to_moflow_rep(input_smiles).detach().cpu().numpy()
        z_initials = []
        ...
        original_mol_rep = smiles_to_transformerm_rep(input_smiles)
        original_mol_rep = F.normalize(original_mol_rep, dim=-1)

        for z_initial in tqdm(z_initials):
            try:
                z = z_initial
                z.requires_grad = True
                z, loss = run_z_optimize(moflow_model, zinc250_atomic_num_list, input_text, z, total_steps, original_mol_rep,
                                         alpha)
                ...
                print(f'Generation of molecule {i + 1} for {input_smiles} failed due to {e}')
                print(traceback.format_exc())
    except Exception as e:
        print(f'Failed to generate molecules for {input_smiles}: {e}')
        return [input_smiles]
