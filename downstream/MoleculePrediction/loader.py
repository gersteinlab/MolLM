...
class MoleculeDataset(InMemoryDataset):
    ...
    def get(self, idx):
        ...
        for key in self.data.keys:
            if key == 'pos':
                data['pos'] = torch.from_numpy(self.data['pos'][idx]).to(torch.float32)
                continue
            item, slices = self.data[key], self.slices[key]
            s = list(repeat(slice(None), item.dim()))
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            smiles_list, rdkit_mol_objs, labels = \
                _load_hiv_dataset(self.raw_paths[0])
            for i in range(len(smiles_list)):
                if (os.path.exists(os.join("dataset", "hiv", "torch_checkpoints", f"{i}.pt"))):
                    data = torch.load(os.join("dataset", "hiv", "torch_checkpoints", f"{i}.pt"))
                ...
                torch.save(data, os.join("dataset", "hiv", "torch_checkpoints", f"{i}.pt"))
