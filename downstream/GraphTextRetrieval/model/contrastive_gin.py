...
class GINSimclr(pl.LightningModule):
    ...
    def forward_original(self, features_graph, features_text):
        ...
        labels = torch.arange(batch_size, dtype=torch.long, device=self.device)  # size is B
    ...
    def forward(self, collated, text, mask):
        if collated is None:
            return self.text_proj_head(self.text_encoder(text, mask))
    ...
    def training_step(self, batch, batch_idx):
        try:
            aug1, aug2, aug3, text1, mask1, text2, mask2, text3, mask3 = batch
            ...
            # 3 * 3 = 9 normal losses
            _, _, loss11 = self.forward_original(graph1_rep, text1_rep)
            _, _, loss12 = self.forward_original(graph1_rep, text2_rep)
            _, _, loss13 = self.forward_original(graph1_rep, text3_rep)

            _, _, loss21 = self.forward_original(graph2_rep, text1_rep)
            _, _, loss22 = self.forward_original(graph2_rep, text2_rep)
            _, _, loss23 = self.forward_original(graph2_rep, text3_rep)

            _, _, loss31 = self.forward_original(graph3_rep, text1_rep)
            _, _, loss32 = self.forward_original(graph3_rep, text2_rep)
            _, _, loss33 = self.forward_original(graph3_rep, text3_rep)

            if self.graph_self:
                # 3 choose 2 = 3 self losses
                _, _, loss_graph_self_1 = self.forward_original(graph1_rep, graph2_rep)
                _, _, loss_graph_self_2 = self.forward_original(graph2_rep, graph3_rep)
                _, _, loss_graph_self_3 = self.forward_original(graph1_rep, graph3_rep)

                loss_1 = (loss11 + loss12 + loss13 + loss21 + loss22 + loss23 + loss31 + loss32 + loss33) / 9.0
                loss_2 = (loss_graph_self_1 + loss_graph_self_2 + loss_graph_self_3) / 3.0

                loss = loss_1 + loss_2
            else:
                loss = (loss11 + loss12 + loss13 + loss21 + loss22 + loss23 + loss31 + loss32 + loss33) / 9.0
