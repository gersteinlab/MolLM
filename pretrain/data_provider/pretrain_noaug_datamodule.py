...
class GINPretrainDataModule(LightningDataModule):
    ...
    def custom_collate(self, data):
        try:
            ...
            num_good = 0
            for tuple in data:
                if tuple is not None and tuple[0] is not None:
                    num_good += 1

            aug_mask = torch.zeros((num_good, 4))
            ...
            for tuple in data:
                if tuple is not None and tuple[0] is not None:
                    ...
                    if 0 < len(tuple[1]):
                        og_graphs.append(original_pt)
                        aug_mask[tuple_idx][0] = 1
                    else:
                        og_graphs.append(original_pt)
            ...
            self.number_graph_list(og_graphs)

            og = collator_3d(og_graphs, max_node=max_node, multi_hop_max_dist=multi_hop_max_dist,
                               spatial_pos_max=spatial_pos_max)

            return aug_mask, og, og, og, og, torch.stack(text_list_1), torch.stack(text_list_2), torch.stack(
                text_list_3), \
